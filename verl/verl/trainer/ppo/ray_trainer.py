...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        advantages, returns = core_algos.compute_drgrpo_outcome_advantage(token_level_rewards=token_level_rewards,
                                                                        eos_mask=response_mask,
                                                                        index=index)
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
...
class RayPPOTrainer(object):
    ...
    def __init__(self,
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            self.use_critic = True
                ...
                AdvantageEstimator.RLOO, AdvantageEstimator.DRGRPO
        ]:
    ...
    def _validate_config(self):
        ...
        # critic
        if self.use_critic and not config.critic.use_dynamic_bsz:
            assert real_train_batch_size >= config.critic.ppo_mini_batch_size
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('gen', timing_raw):
                        # log the fact that we are generating
                        pprint(f'Generating batch {self.global_steps}...')
                        gen_batch_output = self.actor_rollout_wg.generate_sequences(gen_batch)
                    pprint(f'Generation done for step {self.global_steps}.')
