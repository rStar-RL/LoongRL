    ...
    for dataset in "${datasets[@]}"; do
        for seq_len in "${seq_lens[@]}"; do
            ...
            while [ $start_index -lt $total_samples ]; do
                ...
                echo input file: /mnt/longcontext/models/siyuan/test_code/longcontext_syth/$dataset/${dataset}_qwen_filtered-num_sample_${total_samples}-max_seq_$seq_len.jsonl
                python examples/data_preprocess/longcontextqa_like_dataset_system.py \
                    --data_source /mnt/longcontext/models/siyuan/test_code/longcontext_syth/$dataset/${dataset}_qwen_filtered-num_sample_${total_samples}-max_seq_$seq_len.jsonl \
                    --local_dir /mnt/longcontext/models/siyuan/rl_datasets/rl_three/system/${dataset}_qwen_filtered_start_idx${start_index}_end_idx${end_index}_seq${seq_len} \
